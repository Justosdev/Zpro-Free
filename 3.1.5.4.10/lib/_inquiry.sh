...
inquiry_options() {
  ...
  printf "  ${GREEN}[1]${NC}  Instalar ZPRO           ${DIM}— Sistema, Node 20, Docker, PostgreSQL, Redis, nginx, SSL, PM2${NC}\n"

  printf "${CYAN_LIGHT}  ATUALIZAÇÃO${NC}  ${DIM}───────────────────────────────────────────────${NC}\n"
  printf "  ${YELLOW}[2]${NC}  Atualizar ZPRO              ${DIM}— Para PM2, substitui arquivos, migrations, reinicia${NC}\n"
  printf "  ${YELLOW}[3]${NC}  Instalar instâncias secund. ${DIM}— Nova instalação ZPRO isolada no mesmo servidor${NC}\n"
  printf "  ${YELLOW}[4]${NC}  Atualizar instâncias secund.${DIM}— Aplica última versão em todas as instâncias${NC}\n"

  printf "${CYAN_LIGHT}  CONFIGURAÇÕES${NC}  ${DIM}─────────────────────────────────────────────${NC}\n"
  printf "  ${DIM}[0]${NC}   Interface de empresa     ${DIM}— Portal auto-cadastro de tenants, domínio e SSL próprios${NC}\n"
  printf "  ${DIM}[5]${NC}   Instalar Webchat         ${DIM}— Widget JS para incorporar em sites externos${NC}\n"
  ...
  printf "  ${DIM}[11]${NC}  Permissões deployzdg     ${DIM}— chown -R deployzdg:deployzdg /home/deployzdg${NC}\n"
  ...
  printf "${CYAN_LIGHT}  FRONTENDONOVO / DOCKER${NC}  ${DIM}──────────────────────────────────────${NC}\n"
  printf "  ${YELLOW}[15]${NC}  frontendNovo (Beta)      ${DIM}— Instalar ou atualizar o novo frontend Next.js${NC}\n"
