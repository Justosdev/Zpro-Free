...
inquiry_options() {
  ...
  printf "  ${GREEN}[1]${NC}  Instalar ZPRO Free      ${DIM}— Sistema, Node 20, Docker, PostgreSQL, Redis, nginx, SSL, PM2${NC}\n"

  printf "${CYAN_LIGHT}  ATUALIZAÇÃO${NC}  ${DIM}───────────────────────────────────────────────${NC}\n"
  printf "  ${YELLOW}[2]${NC}  Atualizar ZPRO Free         ${DIM}— Para PM2, substitui arquivos, migrations, reinicia${NC}\n"
